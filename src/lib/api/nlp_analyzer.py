...
def analyze_text(text):
    ...
    if nv_ratio > 3.0:  # Massive nominal collapse (Classic IA)
        raw_percentage = max(98, raw_percentage + 40)
    elif nv_ratio < 2.5 and human_count >= 1: # High verbal dynamism (Human)
        raw_percentage = min(5, raw_percentage * 0.1)
    
    # Specific edge case targeting
    if human_count >= 2:
        raw_percentage = raw_percentage * 0.15
        
    # Additional safety for extremely high entropy texts like De Prada or Perez Reverte
    ...
    for i, p in enumerate(raw_paragraphs):
        ...
        if p_len < 10:
            ...
            # Simple heuristic for partials to save compute time
